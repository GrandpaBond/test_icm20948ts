...
class ICM20948 {
    ...
    constructor(icmAddress: number, magAddress: number) {
        ...
        // Activate I2C Master?
        //this.useBank(0);
        //i2cWriteByte(this.icmAddress, ICM20948_USER_CTRL, 0x20) // bit 0x20 says:I2C_MST_EN (I2C Master Enabled)
        ...
        this.useBank(3);
        i2cWriteByte(this.icmAddress, ICM20948_I2C_SLV4_ADDR, slv_addr)    // targeting slave0 address,
        i2cWriteByte(this.icmAddress, ICM20948_I2C_SLV4_REG, AK09916_WIA2) // ask for "Who am I" register
        i2cWriteByte(this.icmAddress, ICM20948_I2C_SLV4_CTRL, slv_ctrl)    // go do it!
        ...
        // perform read-modify-write on the registe(this.icmAddress, ICM20948_USER_CTRL, 0, ICM20948_USER_CTRL_I2C_MST_EN)

        // read back the result
        let magId = i2cReadByte(this.icmAddress, ICM20948_EXT_SLV_SENS_DATA_00)
    ...
    */
    ...
    checkForAK09916() {
        let id = -1
        if (this.magIsDirect) {
            id = i2cReadByte(this.icm, AK09916_WIA2)
        } else {
    ...
    useMagDirect() {
        ...
        this.useBank(0)
        i2cRegisterFlags(this.icm, ICM20948_USER_CTRL,  // in the USER_CTRL register...
    ...
    magRegisterFlags(register: number, unsetMask: number, setMask: number) {
        if (this.magIsDirect) {
            (this.mag, register, unsetMask, setMask)
